...
registration_file=("c:/Agent/registration.json")
log_file_location="c:/Agent/log/agent_log.txt"
...
def heartbeat_data():
    ...
    registration_file=open("c:/Agent/registration.json","r")
    
    reg_data=json.load(registration_file)
    UID=uid_grab()
            
    return {"UID":UID,"user_details":user()}
...
while registartion_status=="not registered":
    ...
    if "UID" not in reg_data :
        ...
        try:    
            ...
            send=requests.post("http://10.100.10.2:5000/reg",json=reg_message,timeout=5)
            ...
            try:
                ...
                send=requests.post("http://10.100.10.2:5000/reg_pending",json=pending_data,timeout=5)
...
while registartion_status=="registered":
    ...
    if status=="not_connected":
        ...
        try:
            ...
            #print("start_sending")
            send=requests.post("http://10.100.10.2:5000/userdata",json=data,timeout=15)
    ...
    if status=="connected":
        ...
        try:
            ...
            send=requests.post("http://10.100.10.2:5000/heartbeat",json=heartbeat,timeout=5)
            print(send.json())
            ...
            if send.status_code == 200:
                if rev_status=="online":
                    ...
                    heartbeat_count-=1
                    print(f"Next Posturing after {heartbeat_count} heart messages")
