...
def FortiGate_Allow(device_ip,device_port,device_toke,ip,group):
    ...
    ip_send=requests.post(f"https://{device_ip}:{device_port}/api/v2/cmdb/firewall/address",headers=header,json=payload,verify=False,timeout=20)
    group_payload=payload = {"name": ip}
    group_send=requests.post(f"https://{device_ip}:{device_port}/api/v2/cmdb/firewall/addrgrp/{group}/member",headers=header,json=group_payload,verify=False,timeout=20)
    return group_send.json(),group_send.status_code
def FortiGate_Delete(device_ip,device_port,device_toke,ip,group):
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    api_key="5ghpbxbgwQrj173Hd68bggdf0kkq0h"
    header={"Authorization":f"Bearer {device_toke}"}
    group_send=requests.delete(f"https://{device_ip}:{device_port}/api/v2/cmdb/firewall/addrgrp/{group}/member/{ip}",headers=header,verify=False,timeout=20)
    ip_send=requests.delete(f"https://{device_ip}:{device_port}/api/v2/cmdb/firewall/address/{ip}",headers=header,verify=False,timeout=20)
